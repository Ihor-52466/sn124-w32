...
import numpy as np
Qe7bbc66 = np.array([0.0, 0.0, 1.0], dtype=np.float32)
Qe706e0c = 0.5
Q36d610e = 20.0
Q48b8f42 = 3.141
Q042fe86 = 1 / 50
...
class _q0502db4:
    ...
    def __init__(self, alpha=0.45, beta=0.1, dt=Q042fe86):
...
def _qbf54a18(depth_map: np.ndarray) -> np.ndarray:
...
def _qa6764d8(depth: np.ndarray) -> np.ndarray:
    return Qe706e0c + depth * (Q36d610e - Qe706e0c)

def _q83a0002(camera_vector: np.ndarray, camera_forward: np.ndarray, camera_right: np.ndarray, camera_up: np.ndarray) -> np.ndarray:
    world = (camera_right * float(camera_vector[0]) + camera_up * float(camera_vector[1]) + camera_forward * float(camera_vector[2])).astype(np.float32)
    return _qaac61a8(world)

def _q4626b3c(camera_target: np.ndarray, *, camera_position: np.ndarray, camera_target_is_point: bool, camera_up: np.ndarray | None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    forward = _qaac61a8(target)
    up_guess = Qe7bbc66 if camera_up is None else _qaac61a8(camera_up)
    right = np.cross(forward, up_guess).astype(np.float32)
    if float(np.linalg.norm(right)) <= 1e-08:
        fallback_up = np.array([0.0, 1.0, 0.0], dtype=np.float32)
        right = np.cross(forward, fallback_up).astype(np.float32)
    right = _qaac61a8(right)
    up = _qaac61a8(np.cross(right, forward))
    return (forward, right, up)

def _qa163683(fov_deg: float | Iterable[float]) -> tuple[float, float]:
...
def _qf5c5577(depth_m: np.ndarray, out_h: int, out_w: int) -> np.ndarray:
...
def _q2858b5c(height: int, width: int, working_resolution: int) -> tuple[int, int]:
...
def _q574f35a(height: int, width: int, fov_x_deg: float, fov_y_deg: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    ray_points = np.stack([x_grid * tan_half_x, y_grid * tan_half_y, np.ones_like(x_grid, dtype=np.float32)], axis=-1).astype(np.float32)
    ray_norms = np.linalg.norm(ray_points, axis=-1, keepdims=True)
    ray_dirs = (ray_points / np.maximum(ray_norms, 1e-08)).astype(np.float32)
    return (ray_points, ray_dirs)

def _q734f913(camera_vectors: np.ndarray, camera_forward: np.ndarray, camera_right: np.ndarray, camera_up: np.ndarray) -> np.ndarray:
    ...
    world = (vectors[:, 0:1] * camera_right[None, :] + vectors[:, 1:2] * camera_up[None, :] + vectors[:, 2:3] * camera_forward[None, :]).astype(np.float32)
    norms = np.linalg.norm(world, axis=1, keepdims=True)
    return (world / np.maximum(norms, 1e-08)).astype(np.float32)

def _qbd03d2f(depth_map: np.ndarray, camera_position: np.ndarray, camera_target: np.ndarray, fov_deg: float | Iterable[float], *, current_direction: np.ndarray | None=None, camera_target_is_point: bool=False, camera_up: np.ndarray | None=None, working_resolution: int=49) -> dict:
    depth = _qbf54a18(depth_map)
    depth_m = _qa6764d8(depth)
    fov_x_deg, fov_y_deg = _qa163683(fov_deg)
    camera_forward, camera_right, camera_up_vector = _q4626b3c(camera_target, camera_position=np.asarray(camera_position, dtype=np.float32), camera_target_is_point=camera_target_is_point, camera_up=camera_up)
    pooled_h, pooled_w = _q2858b5c(depth_m.shape[0], depth_m.shape[1], working_resolution)
    pooled_depth_m = _qf5c5577(depth_m, pooled_h, pooled_w)
    ray_points_cam, ray_dirs_cam = _q574f35a(pooled_h, pooled_w, fov_x_deg, fov_y_deg)
    ...
    candidate_world_dirs = _q734f913(candidate_dirs, camera_forward=camera_forward, camera_right=camera_right, camera_up=camera_up_vector)
    if current_direction is None:
        ...
        if float(np.linalg.norm(preferred_candidate)) <= 1e-08:
            preferred_direction = camera_forward
        else:
            preferred_direction = _qaac61a8(preferred_candidate)
    preference_angles = np.arccos(np.clip(candidate_world_dirs @ preferred_direction, -1.0, 1.0)).astype(np.float32)
    obstacle_ranges_sq = np.sum(surface_points_cam.reshape(-1, 3) ** 2, axis=1, dtype=np.float32)
    projections = candidate_dirs @ surface_points_cam.reshape(-1, 3).T
    return {'candidate_dirs': candidate_dirs, 'camera_forward': camera_forward, 'camera_right': camera_right, 'camera_up_vector': camera_up_vector, 'surface_ranges': surface_ranges, 'obstacle_ranges_sq': obstacle_ranges_sq, 'projections': projections, 'preference_angles': preference_angles}

def _qb47f2d6(ctx: dict, *, drone_radius_m: float=0.06, safety_margin_m: float=0.03, preferred_clearance_m: float=3.0, max_lookahead_m: float=8.0) -> np.ndarray:
    ...
    if np.any(relevant_points):
        ...
        if np.any(collision_mask):
            penetration = np.sqrt(np.maximum(effective_radius * effective_radius - lateral_sq[collision_mask], 0.0)).astype(np.float32)
            collision_distances[collision_mask] = np.maximum(proj_rel[collision_mask] - penetration, 0.0)
    ...
    if safe_candidates.size > 0:
        best_order = np.lexsort((-clearance_distance[safe_candidates], preference_angles[safe_candidates]))
    ...
    return _q83a0002(best_camera_direction, camera_forward=ctx['camera_forward'], camera_right=ctx['camera_right'], camera_up=ctx['camera_up_vector'])

def _qaac61a8(vector: np.ndarray, eps: float=1e-08) -> np.ndarray:
...
def qc3b853c(roll: float, pitch: float, yaw: float) -> np.ndarray:
    cr, sr = (np.cos(roll), np.sin(roll))
    cp, sp = (np.cos(pitch), np.sin(pitch))
    cy, sy = (np.cos(yaw), np.sin(yaw))
    rotation_x = np.array([[1, 0, 0], [0, cr, -sr], [0, sr, cr]], dtype=np.float32)
    rotation_y = np.array([[cp, 0, sp], [0, 1, 0], [-sp, 0, cp]], dtype=np.float32)
    rotation_z = np.array([[cy, -sy, 0], [sy, cy, 0], [0, 0, 1]], dtype=np.float32)
    return (rotation_z @ rotation_y @ rotation_x).astype(np.float32)

def _qd17851e(drone_position: np.ndarray, drone_rpy: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    rotation_matrix = qc3b853c(drone_rpy[0], drone_rpy[1], drone_rpy[2])
    forward_direction = _qaac61a8(rotation_matrix @ np.array([1.0, 0.0, 0.0], dtype=np.float32))
    up_guess = _qaac61a8(rotation_matrix @ np.array([0.0, 0.0, 1.0], dtype=np.float32))
    right_direction = np.cross(forward_direction, up_guess).astype(np.float32)
    if float(np.linalg.norm(right_direction)) <= 1e-08:
        right_direction = np.cross(forward_direction, np.array([0.0, 0.0, 1.0], dtype=np.float32)).astype(np.float32)
    right_direction = _qaac61a8(right_direction)
    up_direction = _qaac61a8(np.cross(right_direction, forward_direction))
    camera_position = (np.asarray(drone_position, dtype=np.float32) + forward_direction * Qc786694 + up_guess * Q7383863).astype(np.float32)
...
class DroneFlightController:
    ...
    def __init__(self, *, m_model_path: Optional[Path]=None):
        ...
        self._qb1287a1(m_model_path)
        self._ab_tracker = _q0502db4()
        self.reset()
        self._qc58abc2(nav_model_path)
        self.controller = Qda11131(gains=Qcff110d())

    def _qd3a32de(self, new_action, state, attitude_action=0.0, is_smooth=True, eps=3e-05):
        if not is_smooth:
    ...
    def act(self, observation):
        ...
        if self._mode == 'search' and self._goal_return_mode and (self._last_known_goal_pos is not None):
        ...
        search_area_position = self._qb48ed67(search_area_vector, drone_position)
        ...
        if distance_to_search_area <= Q36d610e or self._mode in ('navigation', 'landing'):
            goal_visibility_prob, predicted_goal_position, predicted_goal_position_cov, predicted_platform_velocity = self._q792ca14(depth, state)
        ...
        if self._mode == 'takeoff':
            ...
            yaw_command = self._q86ad356(yaw_to_search_area / np.pi, drone_rpy)
            z_command = 0.0
            speed_command = 0.35
            min_altitude = 4.5 if self._mountain_flight and self.is_hatt else 1.5
            if not hasattr(self, 'start_state'):
            ...
            if self._landing_platform_position is None:
                if distance_to_search_area > 1.0:
                    ...
                    if abs(yaw_command - drone_rpy[2]) > 0.3 and abs(yaw_command - drone_rpy[2]) < np.pi * 2 - 0.5:
                        ...
                        speed_command = max(speed_command, 0.2)
                        yaw_command = self._q86ad356(yaw_command / np.pi, drone_rpy) * np.pi
                    ...
                    if distance_to_search_area < 1.0 and self.search_stage > 0:
                        self.search_stage += 1
                        yaw_command = self._qfd8eb3f(drone_rpy[2])
                ...
                if float(np.linalg.norm(direction)) > 1e-06:
            ...
            if visible_goal_position is not None:
                ...
                action = np.concatenate([direction_norm, [speed_command, self._q86ad356(yaw_command, drone_rpy)]], dtype=np.float32)
            else:
            ...
            if not is_goal_visible or visible_goal_position is None:
                ...
                if self._goal_lost_steps > _patience:
                    if self.platform_position is not None and self._goal_return_attempts < 3 and (self._track_score >= 3):
            ...
            if visible_goal_position is not None:
                ...
                if self._landing_platform_position is None:
                    ...
                    if dist_drone_to_anchor < 1.5 and dxy < 0.5:
                        self._landing_platform_position[0:2] = 0.8 * self._landing_platform_position[0:2] + 0.2 * goal_position[0:2]
                ...
                yaw_command = np.arctan2(direction_to_current_goal_position[1], direction_to_current_goal_position[0]) / np.pi
        ...
        if self._mode == 'search' and distance_to_search_area > 3.0 or self._mode == 'navigation':
            camera_position, camera_target, _, up_direction, _ = _qd17851e(drone_position, drone_rpy)
            ctx = _qbd03d2f(depth_map=depth, camera_position=camera_position, camera_target=camera_target, fov_deg=Qb794bb0, current_direction=action[0:3], camera_target_is_point=True, camera_up=up_direction, working_resolution=40)
            waypoint_direction = _qb47f2d6(ctx, safety_margin_m=0.5, preferred_clearance_m=10.0, max_lookahead_m=15.0)
            waypoint_direction_close = _qb47f2d6(ctx, safety_margin_m=1.1, preferred_clearance_m=2.0, max_lookahead_m=4.0)
            dir_similarity = 1.0
            ...
            action = self.q8aeaeaf(depth, state, action, direct_goal_direction)
        ...
        if self._inside_goal_zone and dist_raw_3d > 12.0 and (self._mode not in ('search', 'navigation', 'landing')):
            ...
            if _bn > 1e-06:
                ...
                action = np.concatenate([_back_dir, [min(float(action[3]), 0.5), action[4]]]).astype(np.float32)
        if self._last_action is not None:
            slerp_dir_vector = qa4660cb(a=self._last_action[0:3], b=action[0:3], t=slerp_steps)
        ...
        if (self._mode == 'navigation' or self._mode == 'search') and (not self._forward):
            ...
            is_first_order = False
        final_action = self._qd3a32de(action, state, is_smooth=is_first_order)
        _tilt = max(abs(float(drone_rpy[0])), abs(float(drone_rpy[1])))
        _max_v_err = max(0.2, 1.9 - 2.0 / 0.7 * max(0.0, _tilt - 0.3))
        final_action = self._q7f0d6cc(final_action, drone_velocity, max_v_err=_max_v_err, speed_limit=3.0)
        return final_action

    def _q7f0d6cc(self, action_1x5, drone_velocity_world, max_v_err=2.2, speed_limit=3.0):
    ...
    def q8aeaeaf(self, depth, state, action, gaol_direction):
    ...
    def _qb48ed67(self, search_area_vector, drone_position):
        search_direction = search_area_vector / np.linalg.norm(search_area_vector)
        if self.extra_search_vectors is None:
            init_dist = float(np.linalg.norm(search_area_vector))
            _budget = max(0.0, 53.66 - init_dist / 3.0)
            arm = float(np.clip(np.sqrt(_budget * 6.494), 12.0, 18.0))
            self.extra_search_vectors = [np.array([0.0, 0.0, 0.0]), search_direction * arm, qc3b853c(0.0, 0.0, 3 * np.pi / 4) @ search_direction * arm, qc3b853c(0.0, 0.0, 6 * np.pi / 4) @ search_direction * arm, qc3b853c(0.0, 0.0, 1 * np.pi / 4) @ search_direction * arm, qc3b853c(0.0, 0.0, 4 * np.pi / 4) @ search_direction * arm, qc3b853c(0.0, 0.0, 7 * np.pi / 4) @ search_direction * arm, qc3b853c(0.0, 0.0, 2 * np.pi / 4) @ search_direction * arm, qc3b853c(0.0, 0.0, 5 * np.pi / 4) @ search_direction * arm]
    ...
    def _q792ca14(self, depth, state):
        pr, tr, pv, pq = self._q5892e7b(depth, state, state[:3], state[3:6])
        is_visible = pr > 0.8
        if is_visible and self._ab_tracker.initialized:
            elapsed_steps = max(1, self.platform_lost_step + 1)
            predicted = self._ab_tracker.pos + self._ab_tracker.vel * elapsed_steps * Q042fe86
            innovation = float(np.linalg.norm(tr - predicted))
            max_innovation = 3.0 * elapsed_steps * Q042fe86 + 2.0
        ...
        self.qe19ce78(goal_pos, self.reverse_landing_platform)
    ...
    def _q86ad356(self, yaw_command_1, drone_rpy):
        ...
        if diff > 0.0:
            return self._qfd8eb3f(drone_yaw)
        else:
            return self._qed12884(drone_yaw)

    def _qfd8eb3f(self, drone_yaw):
        max_yaw_change = Q48b8f42 * Q042fe86
        new_drone_yaw_angle = drone_yaw + max_yaw_change - 0.0001
    ...
    def _qb1287a1(self, m_model_path: Path):
        ...
        try:
            ...
            raise RuntimeError(f'Failed to load TorchScript goal detector from {m_model_path}.') from exc
        model.eval()
        self._m_model = model
        self._m_device = device
        self._m_state_dim = q1010998

    def _qc58abc2(self, nav_model_path: Path):
        ...
        try:
            ...
            session = ort.InferenceSession(str(nav_model_path), sess_options=sess_opts, providers=['CPUExecutionProvider'])
        except Exception as exc:
            raise RuntimeError(f'Failed to load ONNX navigation model. If the model was exported with external data, keep the referenced .onnx.data file next to {nav_model_path}.') from exc
    ...
    def _q5892e7b(self, depth: np.ndarray, state: np.ndarray, drone_position: np.ndarray, drone_rpy: np.ndarray) -> tuple[float, np.ndarray]:
        depth_input = np.asarray(depth, dtype=np.float32)
        if depth_input.ndim != 3 or depth_input.shape[-1] != 1:
            raise ValueError(f'expected depth shape (H,W,1), got {depth_input.shape}')
        camera_position, camera_target, _, _, _ = _qd17851e(drone_position, drone_rpy)
        state_input = np.asarray(state, dtype=np.float32).reshape(-1)
        if self._m_state_dim is not None and state_input.shape[0] != self._m_state_dim:
            raise ValueError(f'Goal detector state shape mismatch: expected {self._m_state_dim}, got {state_input.shape[0]}')
        ...
        fov_b = np.asarray([Qb794bb0], dtype=np.float32)
        state_b = state_input[None, :]
        dev = self._m_device
        with torch.inference_mode():
            prediction, self.M = self._m_model(torch.from_numpy(depth_b).to(dev), torch.from_numpy(state_b).to(dev), torch.from_numpy(cam_pos_b).to(dev), torch.from_numpy(cam_tgt_b).to(dev), torch.from_numpy(fov_b).to(dev), self.M)
...
class Qda11131:
    ...
    def __init__(self, gains: Qcff110d=None, setpoint: Optional[np.ndarray]=None, error_deadband: float=0.01):
        self.gains = gains or Qcff110d()
        self.setpoint = setpoint if setpoint is not None else np.zeros(3)
        self.error_deadband = error_deadband

    def update(self, current_pos: np.ndarray, setpoint_v: Optional[np.ndarray]=None, feedforward: Optional[np.ndarray]=None) -> np.ndarray:
